// L6_Proiect_Echipa_NastasaCiprian_UrsanAndrei_Parc_Auto.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
public:
    ...
    Masina(const char* m = "", const char* c = "", int a = 0, float pr = 10, int pt = 0, int nr = 0) {
        ...
        try {
            ...
            marca = new char[strlen(m) + 1]; //alocam dimensiunea pentru array de caractere
    ...
    void citireDate() {
        ...
        try {
            ...
            if (cin.fail()) {
                ...
                throw runtime_error("Anul de fabricatie trebuie sa fie un numar intreg.");
            }
            ...
            if (cin.fail()) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                throw runtime_error("Puterea trebuie sa fie un numar.");
            }
            ...
            if (cin.fail()) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                throw runtime_error("Numarul de locuri trebuie sa fie un numar intreg.");
            }
            ...
            if (cin.fail()) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                throw runtime_error("Pretul trebuie sa fie un numar.");
            }
        }
        catch (const exception& e) {
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            throw; // re-aruncăm excepția pentru a fi prinsă și tratată în zona de apel
        }
    }
    //setare parametri
...
public:
    ...
    void adaugaMasina(Masina x) {
        try {
            ...
            delete[] m;//eliberare memorie de la apelarile anterioare

            m = aux; //inlocium datele cu aux in care am introdus obiectul
        }
        catch (std::bad_alloc& e) {
            // Tratarea excepției în cazul în care nu se poate aloca memorie pentru vectorul aux
            std::cout << "Eroare: Nu s-a putut aloca memorie pentru vectorul auxiliar.\n";
        }
        catch (...) {
            // Tratarea oricărei alte excepții neașteptate
            std::cout << "Eroare: O eroare neașteptată a apărut în timpul adăugării mașinii.\n";
        }
    }
};
void afisareMeniu() {
